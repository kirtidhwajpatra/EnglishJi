...
struct Screen06_AgeColorView: View {
    @ObservedObject var manager: OnboardingManager
    @State private var selectedAge: Int = 27
            ...
            VStack(spacing: 24) {
                ...
                HStack(spacing: 6) {
                    ForEach(0..<5) { index in
                        Rectangle()
                            .fill(index < 3 ? Color(hex: "#110037") : Color.black.opacity(0.2))
                            .frame(height: 4)
                            .cornerRadius(2)
                    }
                }
...
#Preview {
    Screen06_AgeColorView(manager: OnboardingManager())
}
